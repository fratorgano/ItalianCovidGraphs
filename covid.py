...
with urllib.request.urlopen("https://raw.githubusercontent.com/pcm-dpc/COVID-19/master/dati-json/dpc-covid19-ita-andamento-nazionale.json") as url:
    ...
    plot_style(ax)
    total_cases.line_plot(title=True)
    ...
    plot_style(ax)
    active_cases.line_plot(title=True)
    ...
    plot_style(ax)
    recovered.line_plot(title=True)
    ...
    plot_style(ax)
    intensive_care.line_plot(title=True)
    ...
    plot_style(ax)
    hospitalized.line_plot(title=True)
